...
$(function(){
 ...
 //   $("input#name").blur(function(e){ alert("name에 있던 포커스를 잃어버렸습니다."); });
   ...
   $("span#emailcheck").click(function(){
       ...
       // "이메일중복확인" 을 클릭했는지 클릭을 안했는지 여부를 알아오기 위한 용도 
       // 여기서 이메일 중복은 내가 로그인한 상태에서 회원가입시 이메일을 그대로 쓰고싶을 때.
	   // 해당 이메일은 중복안될려고 수정함.
	   // 즉 내가 쓰는 이메일은 제외하고, 다른 사람이 쓰는 이메일이 중복인 지 확인
       ...
       $.ajax({
          ...
          success:function(json){
             ...
             if(json.isExists) {
                // 입력한 email 이 현재 다른 사용자가 사용중이라면
                $("span#emailCheckResult").html($("input#email").val() + " 은 현재 다른 사용자가 사용중 이므로 다른 이메일을 입력하세요").css({"color":"red"}); 
                $("input#email").val(""); 
             }
             else {
                // 입력한 email 이 현재 다른사용자가 사용중이 아니라면 
...
//  또는
   ...
   //////////////////////////////////////////////////////////
    ...
    $.ajax({
	  ...
	  type:"post",
	  async:false, // !!반드시 동기 방식이어야 한다. !!!	  
	  dataType:"json",
	  success:function(json){
		 // console.log(json);
		 // {"isExists" : true}   또는   {"isExists" : false}
		 // 새암호가 기존암호와 동일한 경우 true  
		 // 새암호가 기존암호가 다른 경우 false
	 ...
	 if(isNewPwd){ // 새 암호가 기존 암호와 다른 경우
