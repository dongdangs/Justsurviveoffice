...
// "이메일 중복확인"을 클릭했는지 , 클릭을 안했는지 여부를 알아오기 위한 용도


$(function(){ // header1.jsp 속에 jquery lib이 있어서, memberregister에 없어도 가능하다.
	
	$('span.error').hide();
	$('input#name').focus();
	
	// $('input#name').bind('blur',function(){
		// blur가 포커스를 탭 또는 포커스가 아닌 곳 클릭 시 발생하는 이벤트
		// alert('name 에 있던 포커스를 잃어버렸습니다 - 1.');
	// });
	// 또는
	// $('input#name').blur(function(){alert('name 에 있던 포커스를 잃어버렸습니다 - 2.');});
	$('input#name').blur((e)=>{
		
		// event.target 또는 e.target은 실제로 이벤트가 발생한 태그 즉, blur에서 
		// 'input#name'를 말하는데, val로 해당 값을 가져온다.
		// 해당 input에 이름이나 주소등 text작성 시 해당값 가져온다.
		// This는 function이 있어야 쓸 수 있고, 화살표 함수에선 못쓴다. 
		// alert($(e.target).val());
		
		const name = $(e.target).val().trim();
		if(name == ""){ // 입력하지 않거나, 공백한 입력한 경우
	...
	$('input#pwdcheck').blur((e)=>{
		...
		if( $('input#pwd').val() != $(e.target).val() ){ // 암호와 암호 확인 값이 틀린 경우
		...
		$('input#postcode').blur((e)=>{
			...
			// 숫자 5자리만 (숫자 5자리인데 숫자만 되어야 함) 정규표현식 객체 생성
			...
			if(!bool){ // 우편번호 5자리가 정규표현식에 위배된 경우
			...
			else { // 우편번호 5자리가 정규표현식에 위배된 경우 정규표현식에 맞은 경우				
				$('table#tblMemberRegister :input').prop('disabled', false);
				$(e.target).parent().find('span.error').hide();			
			}	
		...
		*/
		...
		// 우편번호를 읽기전용(readonly)로 만들기 | 스크립트로 작성
		$('input#postcode').attr('readonly',true);		
		
		// 주소를 읽기전용(readonly)로 만들기 | 스크립트로 작성
		$('input#address').attr('readonly',true);		
		
		// 참고항목을 읽기전용(readonly)로 만들기 | 스크립트로 작성
		$('input#extraAddress').attr('readonly',true);		
		
		// === "우편번호찾기"를 클릭했을 때, 이벤트 처리하기 ==== //
		$('img#zipcodeSearch').click(function(){ //클릭하면 핸들러
			...
			// $('input#datepicker').datepicker('setDate', '+3D'); 3일 후 
			...
			// === 전체 datepicker 옵션 일괄 설정하기 ===  
			   //     한번의 설정으로 $("input#fromDate"), $('input#toDate')의 옵션을 모두 설정할 수 있다.
			    $(function() {
				 ...
				 });		// 생년월일메 아우스로 값을 변경하는 경우
				 
		
				 // "아이디중복확인" 클릭했을 때 이벤트 처리하기 시작 //
				 	...
				 	$('img#idcheck').click(function(){
				 		...
				 		// 입력하고자 하는 아이디가 데이터베이스 테이블에 존재하는지, 존재하지 않는지 알아와야 한다. 
				 		 ...
				 		 동기는 1번 -> 2번
				 		 비동기는 1번 하다가 2번하다가 1번 이런식으로 유동적이게
				 		 */
						 ...
						 $.ajax({
							...
							data:{"userid":$('input#userid').val()}, // date 속성은 MyMVC/member/Duplicatecheckup 을 전송해야함
							type:"post", // type을 생략하면 "GET"이 되어버림
							async:true,  // async: true 가 비동기 방식을 말한다. async을 생략하더라도 비동기 방식이다
										 // async: false 가 동기 방식이다. 지도를 할때는 반드시 동기방식인 async:false를 사용해야만 지도가 올바르게 나온다.
							success:function(text){
								console.log(text);
								//{"isExists":false}
								
								// text 는 idDuplicateCheck.up 을 통해 가져온 결과물인
								//  "{"isExists":true}" 또는 "{"isExists":false}" 로 되어지는 
								// string 타입의 결과물이다.
								...
								if(json.isExists) {
									...
									$('span#idcheckResult').html( $('input#userid').val() + "은 이미 사용중 이므로 다른 아이디를 입력하세요." ).css({"color":"red"}); // Jquery innerHtml이다.
									$('input#userid').val("");
								}
				 	...
				 	// "아이디중복확인" 클릭했을 때 이벤트 처리하기 끝 //
					
					// 아이디 값이 변경되면 가입하기 버튼 클릭시 "아이디 중복확인" 을 클릭한지 안한지 
					// 알아보기 위한 용도 초기화
				 	...
				 	$('span#emailcheck').click(function(){
						...
						$.ajax ({
							...
							console.log("---- json의 데이터타입 : ", typeof json);
							// ---- json의 데이터타입 :  object
							// dateType으로 인해 무조건 Object만 출력이 된다.
														
							if(json.isExists){ // 대괄호 표기법, 입력한 이메일이 이미 사용중이라면
								$('span#emailCheckResult').html( $('input#email').val() + "은 이미 사용중 이므로 다른 이메일을 입력하세요." ).css({"color":"red"}); // Jquery innerHtml이다.
								$('input#email').val("");
							}
														
							else {	// 입력한 이메일이 존재하지 않는 경우라면
				 	...
				 	// "이메일중복확인" 클릭했을 때 이벤트 처리하기 끝 //
					
					// 이메일 값이 변경되면 가입하기 버튼 클릭시 "이메일 중복확인" 을 클릭한지 안한지 
					// 알아보기 위한 용도 초기화
...
function goRegister(){
	...
	arr_addressInfo.push($('input#postcode').val()); // 배열에 넣는것.
	...
	/* 틀린문장
	arr_addressInfo.forEach(function(item, index, array){
		if(item.trim() == ""){
			alert('우편번호 및 주소를 입력하셔야 합니다.');
			// forEach는 중간에 break가 없다. 무조건 끝까지 간다. break 못씀
		}
	})
	*/
	...
	const radio_checked_length = $('input:radio[name="gender"]:checked').length;
		if(radio_checked_length == 0) { // 0 은 0, 없음은 ""
			alert('성별을 선택하셔야 합니다.');
			return;
	}
	...
	// 이용약관 입력했는지 검사시작
	const agree = $('input:checkbox[id="agree"]:checked').length;
