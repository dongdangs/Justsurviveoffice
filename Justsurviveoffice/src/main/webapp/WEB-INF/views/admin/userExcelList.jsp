...
<div style="display:flex;width:100%;">
  <div style="width:100%; min-height:1100px; margin:auto;">
    <h2 style="margin: 50px 0; display: flex;">대사살 통계정보(차트)</h2>
...
<script type="text/javascript">
  $(function(){
    ...
    $('#btnExcel').click(function(){
    	...
    	form.chart.value = $('#searchType').val();
    	
    	// 현재 차트에서 사용된 연/월 값 가져오기
        // 월별 차트일 때
        if(form.chart.value === "register") {
            form.year.value = window.nowyear || new Date().getFullYear();
            form.month.value = ""; // 월은 필요 없음
        }
        // 일별 차트일 때
        else if(form.chart.value === "registerDay") {
            form.year.value = new Date().getFullYear(); // 필요하면 서버에서 year도 씀
            form.month.value = window.nowmonth || (new Date().getMonth() + 1);
        }
    	
    	// 폼 전송
   	  	form.method = "post";
   	 	form.action = "<%= ctxPath%>/admin/downloadExcelFile";
   		form.submit();
    });
