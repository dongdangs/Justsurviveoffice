...
public class AdminController {
	...
	public String userList(@RequestParam(name="searchType", defaultValue="")  String searchType,
		...
		try {
			Page<Users> pageUsers = adminService.getPageBoard(searchType, searchWord, currentShowPageNo, sizePerPage);
			
			totalPage = pageUsers.getTotalPages();	// 전체 페이지수 개수
			
			if(currentShowPageNo > totalPage) {
				currentShowPageNo = totalPage;
				pageUsers = adminService.getPageBoard(searchType, searchWord, currentShowPageNo, sizePerPage);
			}
			
			totalDataCount = pageUsers.getTotalElements();	// 전체 데이터의 개수
			
			List<Users> usersList = pageUsers.getContent();	// 현재 페이지의 데이터 목록
